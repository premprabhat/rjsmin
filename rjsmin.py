...
def _make_jsmin(python_only=False):
    ...
    space_sub_banged = _re.compile((
        ...
        r'|(?<=%(preregex1)s)'
            ...
            r'(%(space)s*(?:%(newline)s%(space)s*)+'           # 5
                r'(?=%(post_regex_off)s))?'
        r'|(?<=%(preregex2)s)'
            r'(%(space)s*(?:(%(newline)s)%(space)s*)*)'        # 6, 7
            r'(%(regex)s)'                                     # 8
        ...
        r'|(?<=%(id_literal_close)s)'
            ...
            r'(?=%(id_literal_open)s)'
        r'|(?<=%(id_literal)s)(%(space)s+)(?=%(id_literal)s)'  # 11
        r'|(?<=\+)(%(space)s+)(?=\+)'                          # 12
        r'|(?<=-)(%(space)s+)(?=-)'                            # 13
        r'|(%(space)s+)'                                       # 14
        r'|((?:%(newline)s%(space)s*)+)'                       # 15
    ) % locals()).sub
    ...
    def space_subber_banged(match):
        ...
        if groups[0]:
            ...
            return "%s%s%s%s" % (
                keep(keeper, groups[3]),
                groups[4],
                keep(keeper, groups[5] or ''),
                groups[5] and '\n' or '',
            )
        elif groups[8]:
            return "%s%s%s%s%s" % (
                keep(keeper, groups[6]),
                groups[7] and '\n' or '',
                groups[8],
                keep(keeper, groups[9] or ''),
                groups[9] and '\n' or '',
            )
        elif groups[10]:
            return keep(keeper, groups[10]) + '\n'
        elif groups[11] or groups[12] or groups[13]:
            return keep(keeper, groups[11] or groups[12] or groups[13]) or ' '
        else:
            return keep(keeper, groups[14] or groups[15])
...
def jsmin_for_posers(script, keep_bang_comments=False):
    ...
    if not keep_bang_comments:
        ...
        rex = (
            ...
            r'^/*][^*]*\*+)*/))*(?:(?:(?://[^\r\n]*)?[\r\n])(?:[\000-\011\01'
            r'3\014\016-\040]|(?:/\*[^*]*\*+(?:[^/*][^*]*\*+)*/))*)+(?=[^\00'
            r'0-\040&)+,.:;=?\]|}-]))?|(?<=[\000-#%-,./:-@\[-^\140{-~-]retur'
            r'n)((?:[\000-\011\013\014\016-\040]|(?:/\*[^*]*\*+(?:[^/*][^*]*'
            r'\*+)*/))*(?:((?:(?://[^\r\n]*)?[\r\n]))(?:[\000-\011\013\014\0'
            r'16-\040]|(?:/\*[^*]*\*+(?:[^/*][^*]*\*+)*/))*)*)((?:/(?![\r\n/'
            r'*])[^/\\\[\r\n]*(?:(?:\\[^\r\n]|(?:\[[^\\\]\r\n]*(?:\\[^\r\n]['
            r'^\\\]\r\n]*)*\]))[^/\\\[\r\n]*)*/[a-z]*))((?:[\000-\011\013\01'
            ...
            r':/\*[^*]*\*+(?:[^/*][^*]*\*+)*/))*(?:(?:(?://[^\r\n]*)?[\r\n])'
            r'(?:[\000-\011\013\014\016-\040]|(?:/\*[^*]*\*+(?:[^/*][^*]*\*+'
            r')*/))*)+)(?=[^\000-\040"#%-\047)*,./:-@\\-^\140|-~])|(?<=[^\00'
            r'0-#%-,./:-@\[-^\140{-~-])((?:[\000-\011\013\014\016-\040]|(?:/'
            r'\*[^*]*\*+(?:[^/*][^*]*\*+)*/))+)(?=[^\000-#%-,./:-@\[-^\140{-'
            r'~-])|(?<=\+)((?:[\000-\011\013\014\016-\040]|(?:/\*[^*]*\*+(?:'
            r'[^/*][^*]*\*+)*/))+)(?=\+)|(?<=-)((?:[\000-\011\013\014\016-\0'
            r'40]|(?:/\*[^*]*\*+(?:[^/*][^*]*\*+)*/))+)(?=-)|((?:[\000-\011'
        ...
        def subber(match):
            ...
            return (
                ...
                groups[1] or
                groups[2] or
                groups[4] and "%s%s%s%s" % (
                    keep(keeper, groups[3]),
                    groups[4],
                    keep(keeper, groups[5] or ''),
                    groups[5] and '\n' or '',
                ) or
                groups[8] and "%s%s%s%s%s" % (
                    keep(keeper, groups[6]),
                    groups[7] and '\n' or '',
                    groups[8],
                    keep(keeper, groups[9] or ''),
                ...
                groups[10] and (keep(keeper, groups[10]) + '\n') or
                ...
                keep(keeper, groups[14] or groups[15])
            )
